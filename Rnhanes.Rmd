...
- Variable names `data`, `datasub`, `dfsub` are very generic - change?

- Several citation need proper bibtex entries

- Split appendix into separate supplement 

- Move table of `nhanesA` functions from the appendix to the main paper?

- Rethink the additional comments about BMI: Even if binning BMI is more useful, it's not reasonable to fault CDC for giving numerical values of BMI.
...
library("ggplot2")
demo_combined |> transform(cycle = substring(SDDSRVYR, 8, 17)) |>
