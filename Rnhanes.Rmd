...
We **_use_** the `survey` package [@JSSv009i08] to perform these analyses. We provide a simple example in this paper and more extensive examples in the vignettes for the `nhanesA` package.
...
In order to make appropriate estimates we will need to create a survey design object to incorporate the weights into our analysis. It is essential to create the survey design structure prior to doing any subsetting of your data. This ensures that the complex survey design features, such as stratification and clustering, are accurately captured and applied to the entire dataset. **_In the code below, we use tools in the R package `survey` [@JSSv009i08]. We refer the reader to the documentation for that package for specific recommendations. The CDC provides detailed explanations on how to use survey weights in @cdcwgt2023. _**
...
```{r surveydesign}
dfsub = subset(nhanesDesign, RIDAGEYR >= 40) # method in survey package
...
knitr::kable(adjmnsbyEth[,c(2,3)], col.names=c("Adj DBP","SE"))
##all.equal(names(mns), as.character(adjmnsbyEth$RIDRETH1)
both = cbind(adjmnsbyEth[, c(1, 2)], mns[, 2])
knitr::kable(both, row.names=FALSE, col.names=c("Ethnicity", "Adj DBP", "Raw DPB"))
```
